                        ...
                        <table class="table table-bordered table-striped table-sm">
                            ...
                            <tbody>
                                ...
                                @foreach ($modes as $mode)
                                    <tr>
                                        <td>{{$cpt++}}</td>
                                        <td align="center">{{ $mode->mode }}</td>
                                        <td align="center">
                                            <form action="{{route('modes.edit',$mode)}}" accept-charset="UTF-8" method="get">
                                                @csrf
                                                <input id="mode" type="hidden" class="form-control @error('Mode') is-invalid @enderror" name="mode" value={{ $mode->id }} autocomplete="mode" autofocus>
                                        ...
                                        <td align="center">
                                            <form action="{{route('modes.destroy',$mode)}}" accept-charset="UTF-8" method="post">
                                                @csrf
                                                @method('delete')
                                                <input id="mode" type="hidden" class="form-control @error('Mode') is-invalid @enderror" name="mode" value={{ $mode->id }} autocomplete="mode" autofocus>
                            ...
                            <form action="{{ route('modes.create') }}" accept-charset="UTF-8" method="get">
                                @csrf
